...
void try_add_shift_to_rech(std::string_view user, std::string_view data_folder, std::chrono::minutes shift_length, std::string_view comment) {
    constexpr std::string_view rnr_id{"RNr:"};
    size_t rnr_index = comment.find(rnr_id);
    if (rnr_index == std::string_view::npos)
        return;
    // if (shift_length == std::chrono::minutes(0))
    //     return;
    rnr_index += rnr_id.size();
    size_t rnr_end = comment.find_first_not_of("0123456789", rnr_index);
    std::string rnr{comment.substr(rnr_index, rnr_end - rnr_index)};
    auto [res, output]  = run_command("grep -r '\"id\": \\?" + rnr + "' " + std::string{data_folder} + "* -l | head -1");
    for (;output.size() && (output.back() == '\n' || output.back() == ' '); output.pop_back());
    if (res != EXIT_SUCCESS)
        return;
    std::ifstream rech{output};
    nlohmann::json r_json = nlohmann::json::parse(rech);
    uint32_t mins = shift_length.count();
    r_json["entries"] += "";
    r_json["entries"] += ""; 
    r_json["entries"] += "Arbeitszeit"; 
    r_json["entries"] += std::to_string(mins);
    r_json["entries"] += "min";
    r_json["entries"] += "50";
    r_json["entries"] += std::to_string(mins * 50);
    for (int i = 0; i < r_json["entries"].size() / 7; ++i)
        r_json["entries"][i * 7] = std::to_string(i + 1);
    std::string final_string = r_json.dump(1, '\t');
    update_file(user, output, to_bin_span(final_string));
}
}

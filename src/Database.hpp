...
public:
    using Type = std::variant<float, double, int32_t, int64_t, uint32_t, uint64_t, std::string, 
                                std::chrono::system_clock::time_point, std::vector<std::byte>>;
    struct Table{
        // thre might be more header datas coming, currently only columnar is available
        struct GeneralHeader{
            uint64_t magic_num;
            std::array<char, DBTABLEKINDSTRINGLEN> kind;
        };
        struct HeaderDataColumnar: public GeneralHeader{

...
class diff_match_patch {
 ...
 private:
  static void diff_main(const string_t &text1, const string_t &text2, bool checklines, clock_t deadline, Diffs& diffs) {
    ...
    if (text1 == text2) {
      ...
      string_t commonprefix = text1.substr(0, commonlength);
      ...
      string_t commonsuffix = right(textChopped1, commonlength);
 ...
 public:
  static Diffs diff_fromDelta(const string_t &text1, const string_t &delta) {
    ...
    for (typename string_t::const_pointer token = delta.data(); token - delta.data() < (int)delta.length(); token += token_len + 1) {
 ...
 public:
  Patches patch_fromText(const string_t &textline) const {
    ...
    if (!textline.empty()) {
      ...
      while (text - textline.data() < (int)textline.length()) {
        ...
        for (text += text_len + 1; text - textline.data() < (int)textline.length(); text += text_len + 1) {
          if ((text_len = next_token(textline, traits::from_wchar(L'\n'), text)) == 0) continue;
 ...
 private:
  ...
  static int to_int(const string_t& str) { return traits::to_int(str.data()); }
